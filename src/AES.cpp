#include "AES.h"

/*
AES state layout

uint32_t block[4];

1  2  3  4  block[0];
5  6  7  8  block[1];
9  10 11 12 block[2];
13 14 15 16 block[3];

*/

const unsigned char AES::Sbox[256] = {
    };

const unsigned char AES::invSbox[256] = {
    };

const unsigned char AES::mul2[256] = {
    };

const unsigned char AES::mul3[256] = {
    };

const unsigned char AES::mul9[256] = {
    };

const unsigned char AES::mul11[256] = {
    };

const unsigned char AES::mul13[256] = {
    };

const unsigned char AES::mul14[256] = {
    };

const unsigned char AES::Rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

void AES::Subbytes(uint32_t *input)
{
    for (int c = 0; c < 4; c++) {
        uint32_t word = input[c];
        uint8_t b0 = (word >> 24) & 0xFF;
        uint8_t b1 = (word >> 16) & 0xFF;
        uint8_t b2 = (word >> 8) & 0xFF;
        uint8_t b3 = word & 0xFF;

        b0 = Sbox[b0];
        b1 = Sbox[b1];
        b2 = Sbox[b2];
        b3 = Sbox[b3];

        input[c] = (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
    }
}

void AES::invSubbytes(uint32_t *input)
{
    for(int c = 0; c < 4; c++) {
        uint32_t word = input[c];
        uint8_t b0 = (word >> 24) & 0xFF;
        uint8_t b1 = (word >> 16) & 0xFF;
        uint8_t b2 = (word >> 8) & 0xFF;
        uint8_t b3 = word & 0xFF;

        b0 = invSbox[b0];
        b1 = invSbox[b1];
        b2 = invSbox[b2];
        b3 = invSbox[b3];

        input[c] = (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
    }
}

void AES::Shiftrows(uint32_t *input)
{
    uint32_t temp[4];
    memcpy(temp, input, 16);

    input[0] = (temp[0] & 0xff000000) | (temp[1] & 0x00ff0000) | (temp[2] & 0x0000ff00) | (temp[3] & 0x000000ff);
    input[1] = (temp[1] & 0xff000000) | (temp[2] & 0x00ff0000) | (temp[3] & 0x0000ff00) | (temp[0] & 0x000000ff);
    input[2] = (temp[2] & 0xff000000) | (temp[3] & 0x00ff0000) | (temp[0] & 0x0000ff00) | (temp[1] & 0x000000ff);
    input[3] = (temp[3] & 0xff000000) | (temp[0] & 0x00ff0000) | (temp[1] & 0x0000ff00) | (temp[2] & 0x000000ff);
}

void AES::invShiftrows(uint32_t *input)
{
    uint32_t temp[4];
    memcpy(temp, input, 16);

    input[0] = (temp[0] & 0xff000000) | (temp[3] & 0x00ff0000) | (temp[2] & 0x0000ff00) | (temp[1] & 0x000000ff);
    input[1] = (temp[1] & 0xff000000) | (temp[0] & 0x00ff0000) | (temp[3] & 0x0000ff00) | (temp[2] & 0x000000ff);
    input[2] = (temp[2] & 0xff000000) | (temp[1] & 0x00ff0000) | (temp[0] & 0x0000ff00) | (temp[3] & 0x000000ff);
    input[3] = (temp[3] & 0xff000000) | (temp[2] & 0x00ff0000) | (temp[1] & 0x0000ff00) | (temp[0] & 0x000000ff);
}

void AES::Mixcolumns(uint32_t *input)
{
    for (int c = 0; c < 4; c++) {
        uint8_t b0 = (input[c] >> 24) & 0xFF;
        uint8_t b1 = (input[c] >> 16) & 0xFF;
        uint8_t b2 = (input[c] >> 8) & 0xFF;
        uint8_t b3 = input[c] & 0xFF;

        uint8_t r0 = mul2[b0] ^ mul3[b1] ^ b2 ^ b3;
        uint8_t r1 = b0 ^ mul2[b1] ^ mul3[b2] ^ b3;
        uint8_t r2 = b0 ^ b1 ^ mul2[b2] ^ mul3[b3];
        uint8_t r3 = mul3[b0] ^ b1 ^ b2 ^ mul2[b3];

        input[c] = (r0 << 24) | (r1 << 16) | (r2 << 8) | r3;
    }
}

void AES::invMixcolumns(uint32_t *input)
{
    for(int c = 0; c < 4; c++) {
        uint8_t b0 = (input[c] >> 24) & 0xFF;
        uint8_t b1 = (input[c] >> 16) & 0xFF;
        uint8_t b2 = (input[c] >> 8) & 0xFF;
        uint8_t b3 = input[c] & 0xFF;

        uint8_t r0 = mul14[b0] ^ mul11[b1] ^ mul13[b2] ^ mul9[b3];
        uint8_t r1 = mul9[b0] ^ mul14[b1] ^ mul11[b2] ^ mul13[b3];
        uint8_t r2 = mul13[b0] ^ mul9[b1] ^ mul14[b2] ^ mul11[b3];
        uint8_t r3 = mul11[b0] ^ mul13[b1] ^ mul9[b2] ^ mul14[b3];

        input[c] = (r0 << 24) | (r1 << 16) | (r2 << 8) | r3;
    }
}

void AES::AddKey(uint32_t *input, int round)
{
    input[0] = input[0] ^ expkey[round * 4];
    input[1] = input[1] ^ expkey[round * 4 + 1];
    input[2] = input[2] ^ expkey[round * 4 + 2];
    input[3] = input[3] ^ expkey[round * 4 + 3];
}

// input: unsigned int, output: unsigned int
// input: unsigned char, output: unsigned char
// bufsize: int (in bytes only)
bool AES::encrypt(unsigned char *input, unsigned char *output, int bufsize)
{
    if (bufsize % 16 != 0)
    {
        return 0;
    }

    uint32_t *tempinput = new uint32_t[4];
    uint32_t *tempoutput = new uint32_t[4];

    for (int i = 0; i < bufsize; i += 16)
    {
        ucharToUint32(input + i, tempinput, 4);
        encryptBlock(tempinput, tempoutput);
        Uint32ToUchar(tempoutput, output + i, 4);
    }

    delete[] tempinput;
    delete[] tempoutput;

    return 1;
}

// input: unsigned int, output: unsigned int
// input: unsigned char, output: unsigned char
// bufsize: int (in bytes only)
bool AES::decrypt(unsigned char *input, unsigned char *output, int bufsize)
{
    if (bufsize % 16 != 0)
    {
        return 0;
    }

    uint32_t *tempinput = new uint32_t[4];
    uint32_t *tempoutput = new uint32_t[4];
    for (int i = 0; i < bufsize; i += 16)
    {
        ucharToUint32(input + i, tempinput, 4);
        decryptBlock(tempinput, tempoutput);
        Uint32ToUchar(tempoutput, output + i, 4);
    }

    delete[] tempinput;
    delete[] tempoutput;

    return 1;
}

bool AES::encryptBlock(uint32_t *input, uint32_t *output)
{
    memcpy(output, input, 16); // Copy input to output (buffer size is in bytes)
    AddKey(output, 0);
    for (int round = 1; round < NumRounds; round++)
    {
        Subbytes(output);
        Shiftrows(output);
        Mixcolumns(output);
        AddKey(output, round);
    }
    Subbytes(output);
    Shiftrows(output);
    AddKey(output, NumRounds);
    return 1;
}

bool AES::decryptBlock(uint32_t *input, uint32_t *output)
{
    memcpy(output, input, 16); // Copy input to output (buffer size is in bytes)
    AddKey(output, NumRounds);
    invShiftrows(output);
    invSubbytes(output);
    for (int round = NumRounds - 1; round > 0; round--)
    {
        AddKey(output, round);
        invMixcolumns(output);
        invShiftrows(output);
        invSubbytes(output);
    }
    AddKey(output, 0);
    return 1;
}

void AES::KeyExpansion()
{
    uint32_t temp;
    for (int i = 4; i < 44; i++)
    {
        temp = expkey[i - 1];
        if (i % 4 == 0)
        {
            temp = ((Sbox[(temp >> 16) & 0xFF]) << 24) + ((Sbox[(temp >> 8) & 0xFF]) << 16) + ((Sbox[temp & 0xFF]) << 8) + (Sbox[(temp >> 24) & 0xFF]);
            temp = temp ^ (Rcon[i / 4] << 24);
        }
        expkey[i] = expkey[i - 4] ^ temp;
    }
}

bool AES::encryptFile(const char *filein, const char *fileout, int bufsize)
{
  FILE *fin = fopen(filein, "rb");
  FILE *fout = fopen(fileout, "wb");

  if (fin == nullptr)
  {
    cout << "Error: Cannot open input file" << endl;
    return 0;
  }

  if (fout == nullptr)
  {
    cout << "Error: Cannot open output file" << endl;
    return 0;
  }

  unsigned char *inbuf = new unsigned char[bufsize];
  unsigned char *outbuf = new unsigned char[bufsize];
  int buflen;

  while ((buflen = fread(inbuf, 1, bufsize, fin)) == bufsize)
  {
    encrypt(inbuf, outbuf, buflen);
    fwrite(outbuf, 1, buflen, fout);
  }

  // buflen changed after padding
  if(!pad(inbuf, buflen, 16)){
    std::cerr << "Error: Padding failed" << std::endl;
  }
  encrypt(inbuf, outbuf, buflen);
  fwrite(outbuf, 1, buflen, fout);

  delete[] inbuf;
  inbuf = nullptr;
  delete[] outbuf;
  outbuf = nullptr;

  fclose(fin);
  fclose(fout);

  return true;
}

bool AES::decryptFile(const char *filein, const char *fileout, int bufsize)
{
  FILE *fin = fopen(filein, "rb");
  FILE *fout = fopen(fileout, "wb");

  if (fin == nullptr)
  {
    throw "Error: Cannot open input file";
    return 0;
  }

  if (fout == nullptr)
  {
    throw "Error: Cannot open output file";
    return 0;
  }

  unsigned char *inbuf = new unsigned char[bufsize];
  unsigned char *outbuf = new unsigned char[bufsize];
  int buflen;

  while ((buflen = fread(inbuf, 1, bufsize, fin)) == bufsize)
  {
    decrypt(inbuf, outbuf, buflen);
    fwrite(outbuf, 1, buflen, fout);
  }

  // deal with the last block
  decrypt(inbuf, outbuf, buflen);

  // unpad the last block
  unpad(outbuf, buflen, 16);
  fwrite(outbuf, 1, buflen, fout);

  delete[] inbuf;
  inbuf = nullptr;
  delete[] outbuf;
  outbuf = nullptr;

  fclose(fin);
  fclose(fout);

  return true;
}

AES::AES(const char *key)
{
    expkey = new uint32_t[44];
    NumRounds = 10;

    converHexStringToUint32(key, expkey, 4);
    KeyExpansion();
}

AES::~AES()
{
    delete[] expkey;
    expkey = nullptr;
}
